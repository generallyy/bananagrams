	...
	for x in tile_range * 2:
		for y in tile_range * 2:
			...
			if board_state.has(pos):
				...
				visual_tile.set_meta("origin_tile", original_tile)		# this is unnecessary.
				visual_tile.set_meta("origin_pos", pos)
...
func on_swap_tile_clicked(tile):
	...
	if tile.has_meta("origin_pos"):
		board_state.erase(tile.get_meta("origin_pos"))
	tile.queue_free()
...
func generate_array_default() -> Array:
	var pool = []
	for l in ["J", "K", "Q", "X", "Z"]:
		add_letter(pool, l, 2)
